...
from GNN_Architecture import GCNwithMlp      
...
# ------------------------- 5) MODEL / OPTIMISER ----------------------------
model       = GCNwithMlp(input_dim=1, hidden_dims=[64]*4, output_dim=1)
...
for epoch in range(250):
    ...
    if val_loss < best:
        best = val_loss
        torch.save(model.state_dict(), f'{T}_bestGCN.pt')
...
# 7) FINAL EVALUATION                                                         #
# --------------------------------------------------------------------------- #
model.load_state_dict(torch.load(f'{T}_bestGCN.pt'))
