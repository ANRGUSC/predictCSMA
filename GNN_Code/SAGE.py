...
from GNN_Architecture import SAGEwithMlp        

# ------------------------- 1) GLOBAL SETTINGS ------------------------------
T          = 5                    # just for the save-file name
...
# ------------------------- 5) MODEL / OPTIMISER ----------------------------
model       = SAGEwithMlp(input_dim=1, hidden_dims=[32]*3, output_dim=1)
...
for epoch in range(250):
    ...
    if val_loss < best:
        best = val_loss
        torch.save(model.state_dict(), f'{T}_bestSAGE.pt')
...
# 7) FINAL EVALUATION                                                         #
# --------------------------------------------------------------------------- #
model.load_state_dict(torch.load(f'{T}_bestSAGE.pt'))
